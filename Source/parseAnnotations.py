...
def parse_annotation_csv( filepath, delimiter ):
    df = pd.read_csv(filepath, sep=delimiter, header=None)
    df.columns = ['id', 'center_x', 'center_y', 'orientation', 'tl_x', 'tl_y', 'tr_x', 'tr_y', 'bl_x', 'bl_y',
                  'br_x', 'br_y', 'label', 'full', 'occluded']
    return df

def generate_training_images( dataframe, input_dir, output_dir ):
    ...
    for unused, row in dataframe.iterrows():
        ...
        corners = np.round(np.column_stack((row['center_x'], row['center_y'], row['center_x'], row['center_y']))
                          + box_dim  )
    ...
    for i in range(len(bg_chip)):
        ...
        # Use path to get the input/output folders
        image_file = os.path.join( input_dir , (image_string[i] +"_ir.png") )
        output_file = os.path.join( output_dir , (image_string[i] + "." + str(multi_image_counter) + ".bg.png") )
...
######Training Image Generator Script Entry Point##########
if __name__ == "__main__":
    # Define relevant directory/file paths
    data_folder = os.path.abspath(os.path.join(os.path.dirname(os.path.realpath('__file__')),
                                                            "../Data"))
